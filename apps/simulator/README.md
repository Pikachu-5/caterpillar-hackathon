...
The environment strip beside the site map shows weather and temperature, terrain, soil type, and soil moisture. In the standalone demo it uses the local fixture; authenticated sessions refresh it from snapshots and live updates. Weather mode and freshness remain visible, and soil is labeled with its demo/operator/unknown source. These cues describe the supplied environment and do not change simulator movement or imply a live soil sensor.
